...
class _ProfileHeader extends StatelessWidget {
  ...
  const _ProfileHeader({required this.user, required this.isWeb, required this.onAvatarUpdated});
        ...
        CircleAvatar(
          ...
          child: avatarUrl == null
              ? Text(name.isNotEmpty ? name[0].toUpperCase() : 'U',
                  style: const TextStyle(color: AppTheme.primary, fontSize: 32, fontWeight: FontWeight.w800))
              : null,
        ),
        Positioned(
          bottom: 0, right: 0,
...
class _UserInfo extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.center, children: [
      Text(name, style: const TextStyle(color: AppTheme.text, fontSize: 20, fontWeight: FontWeight.w800)),
      const SizedBox(height: 4),
      Text(email, style: const TextStyle(color: AppTheme.hint, fontSize: 13)),
      const SizedBox(height: 6),
      Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        Icon(isVerified ? Icons.verified_rounded : Icons.warning_rounded,
            color: isVerified ? Colors.green : Colors.orange, size: 14),
        const SizedBox(width: 4),
        Text(isVerified ? 'Correo verificado' : 'Correo no verificado',
            style: TextStyle(color: isVerified ? Colors.green : Colors.orange, fontSize: 12, fontWeight: FontWeight.w600)),
      ]),
    ]);
  }
}
...
class _StatsRow extends StatelessWidget {
  ...
  Widget _divider() => Container(height: 30, width: 1, color: AppTheme.border, margin: const EdgeInsets.symmetric(horizontal: 16));
}
...
class _InfoTabState extends State<_InfoTab> {
  ...
  Future<void> _save() async {
    if (_nameCtrl.text.trim().length < 2) { showBS(context, 'El nombre debe tener mínimo 2 caracteres', isError: true); return; }
    setState(() => _isLoading = true);
    try {
      final res = await ProfileService.updateProfile(name: _nameCtrl.text.trim(), phone: _phoneCtrl.text.trim());
      if (!mounted) return;
      if (res.containsKey('error')) { showBS(context, res['error'], isError: true); } else { showBS(context, 'Perfil actualizado'); widget.onUpdated(); }
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 16),
        _ReadonlyField(label: 'Correo electrónico', value: widget.user['email'] ?? '', icon: Icons.email_outlined),
        const SizedBox(height: 16),
        _ReadonlyField(label: 'Número de cédula', value: widget.user['document_id'] ?? 'No registrado', icon: Icons.badge_outlined),
        const SizedBox(height: 16),
...
class _ReadonlyField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Row(children: [
        Icon(icon, color: AppTheme.hint, size: 20),
...
class _SecurityTabState extends State<_SecurityTab> {
  ...
  Future<void> _changePassword() async {
    if (_newPassCtrl.text != _confirmPassCtrl.text) { showBS(context, 'Las contraseñas no coinciden', isError: true); return; }
    if (_newPassCtrl.text.length < 8 || !_newPassCtrl.text.contains(RegExp(r'[A-Z]')) || !_newPassCtrl.text.contains(RegExp(r'[0-9]'))) {
      showBS(context, 'La contraseña debe tener 8+ caracteres, una mayúscula y un número', isError: true); return;
    }
    setState(() => _isLoading = true);
    try {
  ...
  Future<void> _deleteAccount() async {
    final confirm = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
      ...
      title: const Text('⚠️ Eliminar cuenta', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w800)),
      content: const Text('Esta acción es permanente. Se eliminarán todos tus documentos y datos.', style: TextStyle(color: AppTheme.hint)),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancelar', style: TextStyle(color: AppTheme.hint))),
        TextButton(onPressed: () => Navigator.pop(context, true), child: const Text('Eliminar', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w700))),
      ],
    ));
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        if (!isGoogle) ...[
          ...
          BSTextField(label: 'Nueva contraseña', hint: '8+ chars, mayúscula y número', controller: _newPassCtrl, icon: Icons.lock_outline_rounded, obscureText: true),
          const SizedBox(height: 14),
          BSTextField(label: 'Confirmar nueva contraseña', hint: 'Repite la nueva contraseña', controller: _confirmPassCtrl, icon: Icons.lock_outline_rounded, obscureText: true),
          const SizedBox(height: 20),
          SizedBox(width: double.infinity, height: 50,
            child: ElevatedButton(
          ...
          Container(
            ...
            child: const Row(children: [
              ...
              SizedBox(width: 12),
              Expanded(child: Text('Tu cuenta usa Google. La contraseña se gestiona desde tu cuenta Google.', style: TextStyle(color: AppTheme.hint, fontSize: 13))),
            ]),
          ),
          const SizedBox(height: 32),
        ],
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('Eliminar cuenta', style: TextStyle(color: Colors.red, fontWeight: FontWeight.w700)),
            const SizedBox(height: 6),
            const Text('Esta acción eliminará permanentemente tu cuenta y todos tus documentos.', style: TextStyle(color: AppTheme.hint, fontSize: 12)),
            const SizedBox(height: 12),
            OutlinedButton(
...
class _SignatureTabState extends State<_SignatureTab> with SingleTickerProviderStateMixin {
  ...
  // Canvas handlers
  void _onPanStart(DragStartDetails d) { _currentStroke = [d.localPosition]; setState(() => _strokes.add(_currentStroke)); }
  void _onPanUpdate(DragUpdateDetails d) { setState(() => _currentStroke.add(d.localPosition)); }
  void _onPanEnd(DragEndDetails d) { _currentStroke.add(null); }
  void _clearCanvas() => setState(() { _strokes.clear(); _currentStroke = []; });

  // Guardar desde canvas
  Future<void> _saveFromCanvas() async {
    if (_strokes.isEmpty) { showBS(context, 'Dibuja tu firma primero', isError: true); return; }
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        if (_savedSignatureUrl != null) ...[
          const _SectionTitle('Firma actual'),
          const SizedBox(height: 12),
          Container(
            ...
            child: Column(children: [
              Row(children: [
                ...
                const Text('Firma registrada', style: TextStyle(color: Colors.green, fontWeight: FontWeight.w700, fontSize: 13)),
                const Spacer(),
                GestureDetector(
                  onTap: _deleteSignature,
                  child: Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    decoration: BoxDecoration(color: Colors.red.withOpacity(0.08), borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.red.withOpacity(0.2))),
                    child: const Row(children: [Icon(Icons.delete_outline_rounded, color: Colors.red, size: 14), SizedBox(width: 4), Text('Eliminar', style: TextStyle(color: Colors.red, fontSize: 12))]),
                  ),
                ),
              ]),
              const SizedBox(height: 12),
          ...
          const SizedBox(height: 16),
          const _SectionTitle('Actualizar firma'),
          const SizedBox(height: 12),
        ] else ...[
          const _SectionTitle('Crear firma digital'),
          const SizedBox(height: 4),
...
class _SectionTitle extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Text(text, style: const TextStyle(color: AppTheme.hint, fontSize: 11, fontWeight: FontWeight.w700, letterSpacing: 0.8));
  }
}